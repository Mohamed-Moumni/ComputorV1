...
class TestEquation:
    def test_solve_one_degree_equation_equation1(self):
        _degree = 1
        _terms = dict({"coeff": 1.0, "degree": 1}, {"coeff": 1.0, "degree": 0})
        _equation = Equation(_degree, _terms)
        _solution = _equation.solve_one_degree_equation()
        assert _solution.solution == (-1.0) and _solution.degree == 0

    def test_solve_one_degree_equation_equation2(self):
        _degree = 1
        _terms = dict({"coeff": 0.5, "degree": 1}, {"coeff": 4.0, "degree": 0})
        _equation = Equation(_degree, _terms)
        _solution = _equation.solve_one_degree_equation()
        assert _solution.solution == (8.0) and _solution.degree == 0


    def test_solve_one_degree_equation_equation3(self):
        _degree = 1
        _terms = dict({"coeff": 5.0, "degree": 1})
        _equation = Equation(_degree, _terms)
        _solution = _equation.solve_one_degree_equation()
        assert _solution.solution == () and _solution.degree == 0

    def test_solve_one_degree_equation_equation4(self):
        _degree = 1
        _terms = dict({"coeff": -5.0, "degree": 1}, {"coeff": -5.0, "degree": 0})
        _equation = Equation(_degree, _terms)
        _solution = _equation.solve_one_degree_equation()
        assert _solution.solution == (-1.0) and _solution.degree == 0
    ...
    def test_solve_one_degree_equation_equation5(self):
        _degree = 1
        _terms = dict({"coeff": 2.0, "degree": 1}, {"coeff": -55.0, "degree": 0})
        _equation = Equation(_degree, _terms)
        _solution = _equation.solve_one_degree_equation()
        assert _solution.solution == (
            round(55.0/2.0, 2)) and _solution.degree == 0
